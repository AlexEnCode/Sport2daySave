...
struct MapCellView: View {
        ...
        ZStack {
            ...
            Map(position: $position) {
                ForEach(annotations) { annotation in
                    Annotation(annotation.activity.activityTitle, coordinate: annotation.coordinate) {
                        Button {
                            selectedActivity = annotation.activity
                        } label: {
                            CustomActivityPinView(
                                iconName: annotation.activity.sport.icon,
                                count: "\(annotation.activity.activityPlayers)",
                                color: annotation.activity.level.color
                            )
                        }
                    }
    ...
    private func loadCoordinates() async {
        ...
        for (index, activity) in activities.enumerated() {
            ...
            } else if let fetched = await activity.coordinates() {
                coord = fetched
            } else {
